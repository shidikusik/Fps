...
struct Values {
    ...
    float sensitivity = 1.0f; // 0.4 .. 2.0 multiplier (mouse + touch + pad look)
    float volume = 1.0f;      // 0 .. 1 master volume
    bool vibration = true;    // gamepad rumble
};
...
// Directory for config/saves ("" when unavailable, e.g. Android).
