...
**5 уровней, сюжетные катсцены с озвучкой THE MACHINE, 4 оружия,
босс THE WARDEN и бесконечный NG+ цикл после победы.**
...
BLOODRUSH работает на **BLOODENGINE** — нашем собственном компактном
C++20-движке в каталоге [engine/](engine/), который собирается отдельной
статической библиотекой поверх raylib. В `src/` остался только геймплей.
Возможности движка: загрузочная заставка, рендер-пайплайн с леттербоксом
и шейдером света/тумана, пиксельный UI-шрифт (латиница + кириллица),
typewriter-катсцены, частицы, процедурный аудиосинтез, сохраняемые
настройки и двуязычная система строк.

## Сюжет

Земля мертва. **МАШИНА** просыпается с пустыми баками — и находит
альтернативное топливо: кровь. Интро полностью озвучено (процедурная
роботизированная речь, RU + EN). Башня зовёт вниз, слой за слоем:

1. **ДВОР** — открытая арена со ступенчатой башней.
2. **КАТАКОМБЫ** — лабиринт колонн и надземных мостиков.
3. **АЛТАРЬ** — вертикальный шпиль.
4. **ПЕЧЬ** — железные кольца жара и дымовые трубы.
5. **ТРОН** — парадная лестница к **ХРАНИТЕЛЮ**.

Каждый слой — 4 волны, пятый заканчивается боссом. Убей Хранителя —
...
**Геймпады** работают во всех версиях игры (Linux, Windows, Android по
USB/Bluetooth). DualShock 4 в приоритете: левый стик — движение, правый —
прицел, CROSS — прыжок, L1 — рывок, R1 — подкат/удар, R2 — огонь,
L2 — альт-огонь, SQUARE — смена оружия, OPTIONS — пауза. **Вибрация**
поддерживается (Linux — через force-feedback ядра, Windows — через XInput)
и включается в настройках. При старте загружается
встроенная база маппингов SDL, поэтому геймпады работают корректно
даже по Bluetooth (например, DualShock 4 по Bluetooth). Любое действие (клавиатура, мышь, геймпад)
**переназначается** в Настройки → Управление, сохраняется в
`~/.config/bloodrush.binds`.

**Прогресс сохраняется** на входе в каждый слой (и на каждом круге NG+),
так что можно выйти в любой момент и выбрать **ПРОДОЛЖИТЬ** в меню —
игра продолжится с последнего слоя. Смерть завершает забег и стирает
чекпоинт. Сохранение — в `~/.config/bloodrush.save`.
...
| S (в меню) | настройки: язык, чувствительность, громкость, вибрация, управление |
| Геймпад | полная поддержка (DualShock 4 в приоритете): левый стик — движение, правый — обзор, все кнопки переназначаются |
| ESC | пауза |
| F11 | полный экран |

Настройки (язык / чувствительность / громкость) — кликабельные, что
особенно удобно на Android — и сохраняются в `~/.config/bloodrush.cfg`.
Язык действует на меню, HUD, катсцены и тач-кнопки (стайл-попапы
остаются английскими — это стиль). Кириллицу рисует вшитый пиксельный шрифт
Press Start 2P (лицензия OFL) — файлов ассетов по-прежнему ноль.
...
- **THE WARDEN** — босс на Троне: прыжки с ударной волной
  из сбиваемых снарядов, полоса здоровья, корона.

Каждый уровень — 4 волны, пятый заканчивается боссом. После победы —
...
| `engine/particles.*` | кровь-кубики и искры с физикой |
| `engine/cutscene.*` | letterbox-катсцены с typewriter-текстом |
| `src/touch.*` | тач-управление для Android (стик, обзор, кнопки) |
| `engine/sounds.*` | процедурная генерация всех звуков при старте |
| `engine/shading.h` | направленный свет + туман глубины (GLSL 330 / ES 100) |
