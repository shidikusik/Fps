...
| L (в меню) | язык: English / Русский (есть и кликабельная кнопка) |
| ESC | пауза |
| F11 | полный экран |

Выбор языка действует на меню, HUD, катсцены и тач-кнопки (стайл-попапы
и ранги остаются английскими — это стиль) и сохраняется в
`~/.config/bloodrush.lang`. Кириллицу рисует вшитый пиксельный шрифт
